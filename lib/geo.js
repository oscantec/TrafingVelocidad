...
export function findNodeCrossings(node, points, threshold) {
  // 1. Collect every track point that's inside the threshold disc.
  const near = [];
  for (let i = 0; i < points.length; i++) {
    const d = haversineDistance(node, points[i]);
    if (d <= threshold) near.push({ i, d });
  }
  if (!near.length) return [];

  // 2. Cluster those samples by timestamp gap. Two samples that sit
  //    fewer than MIN_GAP_MS apart belong to the same visit, even if
  //    the track briefly left the threshold disc between them (e.g.
  //    orbit around the corner). The crossing for the cluster is the
  //    sample closest to the node.
  const MIN_GAP_MS = 75_000; // 75 s between independent visits
  const tsOf = (k) => points[k].timestamp ?? k;

  const crossings = [];
  let clusterStart = 0;
  for (let k = 1; k <= near.length; k++) {
    const endCluster = (k === near.length) ||
                       (tsOf(near[k].i) - tsOf(near[k - 1].i) >= MIN_GAP_MS);
    if (!endCluster) continue;
    // near[clusterStart..k-1] is one cluster — keep the closest sample.
    let best = near[clusterStart];
    for (let j = clusterStart + 1; j < k; j++) {
      if (near[j].d < best.d) best = near[j];
    }
    crossings.push({ trackIndex: best.i, distance: best.d });
    clusterStart = k;
  }
  return crossings;
}
