...
/**
 * Compass bearing from p1 to p2, degrees in [0, 360). 0 = north.
 */
...
/**
 * Turn a bearing into an 8-wind cardinal label ("N", "NE", "E", …).
 */
...
/**
 * "FROM - TO" label for direction of travel between two points
 * (e.g. "S - N"). Returns empty string if either point is missing.
 */
export function directionLabel(fromPt, toPt) {
  if (!fromPt || !toPt) return '';
  const b = bearing(fromPt, toPt);
  return `${cardinalFromBearing((b + 180) % 360)} - ${cardinalFromBearing(b)}`;
}

/**
 * Return every time a track passes the given control node. A "crossing"
 * starts when the track dips below `threshold`, and is only closed when
 * the track truly leaves — it has to climb past `2 * threshold` metres
 * from the node again. Without that hysteresis a recorrido that loops
 * around the same corner would register three or four bogus crossings
 * while it's still hovering inside / just-outside the threshold.
 *
...
export function findNodeCrossings(node, points, threshold) {
  const raw = [];
  const escape = threshold * 2;
  let state = 'out'; // 'out' | 'near' | 'leaving'
  let bestIdx = -1;
  let bestDist = Infinity;

  const close = () => {
    if (bestIdx >= 0) raw.push({ trackIndex: bestIdx, distance: bestDist });
    state = 'out'; bestIdx = -1; bestDist = Infinity;
  };

  for (let i = 0; i < points.length; i++) {
    const d = haversineDistance(node, points[i]);
    if (d <= threshold) {
      if (state === 'out') { state = 'near'; bestIdx = i; bestDist = d; }
      else {
        if (d < bestDist) { bestIdx = i; bestDist = d; }
        state = 'near'; // re-enter from 'leaving' without ever escaping
      }
    } else if (d >= escape) {
      if (state !== 'out') close();
    } else if (state === 'near') {
  ...
  // Post-merge: two crossings reported within `MIN_GAP_MS` of each other
  // are the same visit. Keep whichever is closest to the node. This
  // handles the case where a route loops around the control point and
  // the hysteresis alone is not enough (e.g. it orbits between
  // threshold and 2*threshold).
  const MIN_GAP_MS = 20000;
  const merged = [];
  for (const c of raw) {
    const ts = points[c.trackIndex]?.timestamp ?? c.trackIndex;
    const prev = merged[merged.length - 1];
    const prevTs = prev ? (points[prev.trackIndex]?.timestamp ?? prev.trackIndex) : -Infinity;
    if (prev && Math.abs(ts - prevTs) < MIN_GAP_MS) {
      if (c.distance < prev.distance) merged[merged.length - 1] = c;
      continue;
    }
    merged.push(c);
  }
