...
const APP_NAME = 'velocidades';   // qué módulo verificamos en user_metadata.apps
...
function userApps(user) {
  const apps = user && user.user_metadata && user.user_metadata.apps;
  return Array.isArray(apps) ? apps : [];
}

/** ¿Tiene esta cuenta permiso para entrar a Velocidades? */
function hasAccessToThisApp(user) {
  if (!user) return false;
  if (userRole(user) === 'superadmin') return true;
  return userApps(user).includes(APP_NAME);
}
...
export async function checkAuth() {
  ...
  if (!hasAccessToThisApp(session.user)) {
...
export async function login(email, password) {
  ...
  if (!hasAccessToThisApp(data.user)) {
    await supabase.auth.signOut();
    const e = new Error('Tu cuenta no tiene acceso al módulo de Velocidades. Pide a un Super Admin que te lo asigne.');
    e.code = 'no_access';
    throw e;
  }
