...
export function parseTrackContent(text, hint) {
  ...
  if (lower.includes('.gpx') || lower.includes('.jsp')) return parseGPX(trimmed);
  if (lower.includes('.kml')) return parseKML(trimmed);
  if (lower.includes('.json') || lower.includes('.geojson')) return parseGeoJSON(trimmed);

  // gpxscan and similar viewers dump the track as JS objects (no <gpx> tag),
  // e.g. `var gpx_data = { "00:00:00": {lat:..,lon:..,ele:..,speed:..}, … }`.
  // parseGPX's regex fallback (Pattern C) already handles those, so try it.
  if (/\b(?:gpx_data|chart_data|current_time)\s*[:=]/i.test(trimmed)
      || /lat\s*:\s*-?[\d.]+[\s\S]{0,80}lon\s*:\s*-?[\d.]+/i.test(trimmed)) {
