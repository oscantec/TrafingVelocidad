...
/**
 * Last-resort regex extractor — pulls trkpt points from any text,
 * even if the XML isn't valid (JSP residue, HTML wrapper, broken schema).
 */
function regexExtractGPX(text) {
  const points = [];
  const re = /<trkpt\b[^>]*\blat\s*=\s*["']([-\d.]+)["'][^>]*\blon\s*=\s*["']([-\d.]+)["'][^>]*>([\s\S]*?)<\/trkpt>|<trkpt\b[^>]*\blat\s*=\s*["']([-\d.]+)["'][^>]*\blon\s*=\s*["']([-\d.]+)["'][^>]*\/>/gi;
  let m;
...
export function parseGPX(xmlString) {
  ...
  // Attempt 3: regex fallback — works even on very broken documents
  const fallback = regexExtractGPX(cleaned);
  if (fallback.points.length === 0) {
    throw new Error('GPX sin puntos de track (ni por XML ni por regex)');
  }
  return fallback;
}
