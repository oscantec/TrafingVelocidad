...
export function parseTrackContent(text, hint) {
  ...
  // JSON / GeoJSON
  if (trimmed.startsWith('{') || trimmed.startsWith('[') || lower.endsWith('.json') || lower.endsWith('.geojson')) {
    const data = JSON.parse(trimmed);
    if (data.type === 'FeatureCollection' || data.type === 'Feature' || data.geometry) {
      return parseGeoJSON(data);
    }
    // Legacy app JSON: { points: [...] } or raw array
  ...
  // KML
  if (trimmed.includes('<kml') || lower.endsWith('.kml')) return parseKML(trimmed);

  // GPX (default for XML)
  if (trimmed.includes('<gpx') || trimmed.startsWith('<?xml') || lower.endsWith('.gpx') || lower.endsWith('.xml')) {
    return parseGPX(trimmed);
  }

  throw new Error('Formato no reconocido. Usa GPX, KML, GeoJSON o JSON.');
}
