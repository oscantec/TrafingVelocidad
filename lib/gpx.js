...
function regexExtractGPX(text) {
  ...
  // Pattern C: JSON arrays embedded in JS, e.g. [{"lat":4.6,"lng":-74.1}, …]
  if (points.length === 0) {
    const jsonObj = /\{\s*["']?lat["']?\s*:\s*([-\d.]+)\s*,\s*["']?(?:lng|lon|longitude)["']?\s*:\s*([-\d.]+)(?:[^}]*?["']?(?:alt|altitude|ele)["']?\s*:\s*([-\d.]+))?[^}]*?\}/gi;
    let j, t = Date.now();
    while ((j = jsonObj.exec(text)) !== null) {
      const lat = parseFloat(j[1]), lng = parseFloat(j[2]);
      if (isNaN(lat) || isNaN(lng)) continue;
      points.push({
        lat, lng,
        altitude: j[3] ? parseFloat(j[3]) : null,
        speed: null,
        timestamp: t++,
      });
    }
