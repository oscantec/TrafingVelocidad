 ...
 * Retorna { success, tramoId } o { success:false, error, missing? }.
 */
export async function saveTramoComplete({ tramoName, points, subtramos }) {
  const supabase = getClient();
  if (!supabase) return { success: false, error: 'Cliente Supabase no cargado' };
  ...
  try {
    // 1. Tramo (sin corredor — cada nodo lleva el suyo).
    const { data: tramo, error: tErr } = await supabase
      .from('tramos').insert({ name: tramoName.trim() }).select().single();
    ...
    return { success: true, tramoId: tramo.id };
  } catch (err) {
    return { success: false, error: err.message || String(err) };
  }
}
