...
/**
 * Lista todos los puntos de control guardados.
 * Retorna {ok, points, missing?} — missing=true si la tabla no existe.
 */
export async function listControlPoints() {
  if (!supabase) return { ok: false, points: [], error: 'Cliente Supabase no cargado' };
  const { data, error } = await supabase
    .from('control_points')
    .select('*')
    .order('created_at', { ascending: false });
  if (error) {
    const missing = /control_points/.test(error.message) && /could not find/i.test(error.message);
    return { ok: false, points: [], error: error.message, missing };
  }
  return { ok: true, points: data || [] };
}

/**
 * Inserta un punto de control.
 */
export async function saveControlPoint({ name, lat, lng }) {
  if (!supabase) return { success: false, error: 'Cliente Supabase no cargado' };
  const { data, error } = await supabase
    .from('control_points')
    .insert({ name, lat, lng })
  ...
  if (error) {
    const missing = /control_points/.test(error.message) && /could not find/i.test(error.message);
    return { success: false, error: error.message, missing };
  }
 ...
 */
export async function deleteControlPoint(id) {
  if (!supabase) return { success: false, error: 'Cliente Supabase no cargado' };
