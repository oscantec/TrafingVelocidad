...
function isMissingTable(error) {
  ...
  return /(could not find.*table|relation.*does not exist|schema cache)/i.test(msg) &&
         /(corridors|tramos|control_points)/i.test(msg);
}
...
/**
 * Guarda un tramo completo en una sola llamada:
 *   - Reutiliza o crea el corredor (match por nombre).
 *   - Crea el tramo dentro del corredor.
 *   - Inserta todos los puntos con seq=0..N.
 * Retorna { success, corridorId, tramoId } o { success:false, error, missing? }.
 */
export async function saveTramoComplete({ corridorId, corridorName, tramoName, points, subtramos }) {
  const supabase = getClient();
  if (!supabase) return { success: false, error: 'Cliente Supabase no cargado' };
  if (!tramoName) return { success: false, error: 'Falta el nombre del tramo' };
  if (!points || points.length === 0) return { success: false, error: 'No hay puntos para guardar' };

  try {
    // 1. Corredor: buscar/crear si viene por nombre
    let cid = corridorId || null;
    if (!cid) {
      const name = (corridorName || '').trim();
      if (!name) return { success: false, error: 'Falta el corredor' };
      const { data: existing } = await supabase
        .from('corridors').select('id').eq('name', name).maybeSingle();
      if (existing) {
        cid = existing.id;
      } else {
        const { data: created, error: cErr } = await supabase
          .from('corridors').insert({ name }).select('id').single();
        if (cErr) return { success: false, error: cErr.message, missing: isMissingTable(cErr) };
        cid = created.id;
      }
    }

    // 2. Tramo
    const { data: tramo, error: tErr } = await supabase
      .from('tramos').insert({ corridor_id: cid, name: tramoName.trim() }).select().single();
    if (tErr) return { success: false, error: tErr.message, missing: isMissingTable(tErr) };

    // 3. Puntos en batch y nos guardamos su mapping local-id -> cloud-id para
    ...
    // 4. Subtramos (opcional). start_ref / end_ref guardan '__start__',
    ...
    return { success: true, corridorId: cid, tramoId: tramo.id };
  } catch (err) {
    return { success: false, error: err.message || String(err) };
  }
}
...
export function buildTrackShareUrl(track, origin) {
  ...
  const tag   = ['Trafing', stamp, slug].filter(Boolean).join('_');
